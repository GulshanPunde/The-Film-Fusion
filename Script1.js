const API_KEY = "24d5a217669f1fb066c077d7ff21e5db";
...
async function showPopUp(card) {
  ...
  popupContainer.style.background = `linear-gradient(rgba(0, 0, 0, 0.6), rgba(0, 0, 0, 1)),
  ...
  heart_icon.addEventListener("click", () => {
    if (heart_icon.classList.contains("change-color")) {
      ...
      removeFromFavorites(movie.id); // Remove from favorites if already favorited
    } else {
  ...
  // Check if movie is already in favorites and update heart icon
...
async function addTrendingMoviestoDOM() {
  ...
  //   const displayMovies = data.slice(0, 12);
...
document.addEventListener("DOMContentLoaded", () => {
  // Load favorites from localStorage on DOM load
  const storedFavorites = JSON.parse(localStorage.getItem("favorites")) || [];
  favorites = storedFavorites;
});

function addFavoriteMoviestoDOM() {
    let resultArr = storedFavorites.map((m) => {
      return `
      <div class="card" data-id="${m.id}">
              <div class="img">
                <img src="${imagePath + m.poster_path}" alt="" />
